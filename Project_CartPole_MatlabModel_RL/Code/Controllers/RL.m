...
if doTraining
    ...
    % 加载预训练 agent 并覆盖
    S = load("Agent495.mat", "saved_agent");
...
% 使用generatePolicyfunction函数将DDPG中agent的actor部分提取出来，保存为agentData.mat文件
generatePolicyFunction(agent)
% 同时生成evaluatePolicy.m函数文件，将作为代码自动生成的接口函数
% evaluatePolicy将接收观测量作为输入，输出动作值
